<script>
  ...
  import { brl, nf } from '../lib/engine.js';
  ...
  function selectPreset(p) {
    rule = { base: p.base, modo: { tipo: p.tipo, valor: p.def } };
  }

  function setPresetValue(p, raw) {
    const v = parseFloat(raw);
    if (isNaN(v)) return;
    rule = { ...rule, base: p.base, modo: { tipo: p.tipo, valor: v } };
  }

  // Avançado — faixas
  function addFaixa() {
    rule.faixas = [...(rule.faixas ?? []), { ate: 10000, valor: 2 }];
    rule = rule;
  }
  function removeFaixa(i) {
    rule.faixas = rule.faixas.filter((_, j) => j !== i);
    rule = rule;
  }
  function setFaixa(i, field, raw) {
    const v = parseFloat(raw);
    if (isNaN(v)) return;
    rule.faixas[i][field] = v;
    rule = rule;
  }

  // Preview da comissão no pedido atual
...
<div class="back" role="presentation" on:click={() => dispatch('close')}>
  <div class="modal" role="dialog" aria-modal="true" on:click|stopPropagation on:keydown|stopPropagation>
    ...
    <!-- Cabeçalho -->
    <div class="mh">
      <div><b>Comissão do vendedor</b><div class="sub">Regra global · cada item pode sobrescrever no futuro</div></div>
            ...
            <table class="faixas">
              ...
              <tbody>
                {#each rule.faixas as f, i}
                  <tr>
                    <td><input class="fnum" type="number" step="100" value={f.ate}
                        on:change={(e) => setFaixa(i, 'ate', e.target.value)} /></td>
                    <td><input class="fnum" type="number" step="0.1" value={f.valor}
                        on:change={(e) => setFaixa(i, 'valor', e.target.value)} /></td>
      ...
      {#if $order.items.length}
        <div class="preview">
          ...
          {#each previewRows as r, i}
            <div class="prev-row">
              <span class="prev-lbl">{i + 1}. {r.label}</span>
              <span class="prev-val">{brl(r.comm)}</span>
            </div>
          {/each}
          <div class="prev-tot">
            <span>Total comissão</span>
            <b>{brl(previewTotal)}</b>
          </div>
        </div>
      {:else}
...
<style>
  ...
  .modal { background: var(--panel); border-radius: 12px; width: 100%; max-width: 560px; max-height: 88vh; display: flex; flex-direction: column; overflow: hidden; }
  ...
  .pval { width: 60px; height: 28px; border: 1px solid var(--amber); border-radius: 5px; padding: 0 6px; text-align: right; font-family: var(--mono); font-size: 13px; background: #fff; }
  ...
  /* Rodapé */
  .mf { display: flex; align-items: center; gap: 10px; padding: 12px 18px; border-top: 1px solid var(--line); flex-wrap: wrap; }
